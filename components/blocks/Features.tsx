  ...
  return (
    <section className="py-20 md:py-32 max-w-5xl mx-auto px-6 space-y-12">
      <div className="text-center mb-20">
        ...
        {data.subtitle && (
          <p 
            className={`${isDark ? 'text-slate-400' : 'text-slate-600'} max-w-2xl mx-auto text-lg md:text-xl font-medium leading-relaxed mt-6`}
            style={getTextStyle(data, 'subtitle')}
          >
            {data.subtitle}
          </p>
            ...
            <div key={i} className={`p-8 md:p-12 rounded-[2.5rem] border transition-all group relative overflow-hidden ${isDark ? 'bg-white/5 border-white/10 hover:bg-white/[0.08]' : 'bg-white border-slate-100 shadow-xl shadow-slate-200/50 hover:-translate-y-1'}`}>
              <div className="relative z-10 flex flex-col gap-6">
                <div className="flex items-center gap-4">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center border ${isDark ? 'bg-pink/10 border-pink/20' : 'bg-blue-50 border-blue-100'}`} style={data.accentColor ? { color: data.accentColor, borderColor: `${data.accentColor}40`, backgroundColor: `${data.accentColor}10` } : {}}>
                     <span className={`text-xl font-black ${isDark ? 'text-pink' : 'text-blue-600'}`} style={data.accentColor ? { color: data.accentColor } : {}}>{i + 1}</span>
                  </div>
                  <h3 className={`text-xl md:text-2xl font-bold tracking-tight ${isDark ? 'text-white' : 'text-slate-800'}`}>{f.t || f.title}</h3>
                </div>
                <p className={`${isDark ? 'text-slate-400' : 'text-slate-500'} text-sm md:text-base font-medium leading-relaxed`}>{f.d || f.desc}</p>
              </div>
            </div>
          );
        })}
